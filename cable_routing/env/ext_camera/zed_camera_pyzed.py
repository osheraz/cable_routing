...
class Zed:
    def __init__(
        ...
        self.init_params.camera_image_flip = (
            sl.FLIP_MODE.ON if flip_mode else sl.FLIP_MODE.OFF
        )
        ...
        if resolution == "720p":
            ...
            raise ValueError(
                "Only 720p, 1080p, and 2k resolutions are supported by ZED."
            )
        ...
        for attempt in range(max_retries):
            if self.cam.open(self.init_params) == sl.ERROR_CODE.SUCCESS:
                print("ZED camera successfully connected.")
                break
            else:
                print(f"Attempt {attempt + 1}/{max_retries}: Failed to connect to ZED camera. Retrying in {1.0} seconds...")
                time.sleep(1.0)
        else:
            raise RuntimeError("Failed to open ZED camera after multiple attempts.")
    ...
    def start_record(self, out_path):
        ...
        recording_parameters = sl.RecordingParameters(
            out_path, sl.SVO_COMPRESSION_MODE.H264
        )
