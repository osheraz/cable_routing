        ...
        for coordinate in cable.get_points():
            if self.board[coordinate[0]][coordinate[1]] == ".":
                ...
                self.board[coordinate[0]][coordinate[1]] = (
                    self.board[coordinate[0]][coordinate[1]] + f", {cable.get_id()}"
                )
    ...
    def suggest_modifications(self, goal_configuration):
        ...
        for cable_id in goal_configuration:
            ...
            if cable_id in self.cables:
                ...
                instructions.append(
                    self.pairwise_correct(
                        self.cables[cable_id].get_keypoints(),
...
class Cable:
    ...
    def __init__(self, coordinates, id=None, environment=None):
        ...
        Instantiate an instance of our cable object.
                ...
                Parameter(s):
                        ...
                        environment (Levenshtein:), optional: The environment that the cable is a part of, can be used to update the cable's relevant keypoints
    ...
    def update_keypoints(self, environment=None):
        ...
        Update the set of the keypoints for the cable, either using a particular "environment" of choice, or using the environment that was assigned upon instantiation.
                ...
                Parameter(s):
                        environment (Levenshtein:), optional: The environment we would like to use for updating the cable's keypoints, leave blank if we would like to use the instatiated environment
        ...
        if environment != None:
            ...
            self.keypoints = [
                point
                for point in self.all_coordinates
                if point in self.environment.key_locations
            ]
...
if __name__ == "__main__":
    ...
    for k in range(3):
        ...
        for i in range(random.randint(6, 10)):
            ...
            if (
                new_pos[0] in range(0, 10)
                and new_pos[0] in range(0, 10)
        ...
        [
            my_environment.add_keypoint(BoardFeature(position))
            for position in cable1.all_coordinates
        ]
