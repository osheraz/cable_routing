...
class BRIOSubscriber:
    def __init__(self, topic_name='/camera/camera_0/image_raw', queue_size=1):
        ...
        self.image_sub = rospy.Subscriber(self.topic_name, Image, self.callback, queue_size=queue_size)
    ...
    def _check_rgb_ready(self):
        ...
        self.latest_frame = None
        rospy.logdebug(
            "Waiting for '{}' to be READY...".format(self.topic_name))
        ...
        while self.latest_frame is None and not rospy.is_shutdown():
            try:
                ...
                rospy.logdebug(
                    "Current '{}' READY=>".format(self.topic_name))
                self.start_time = rospy.get_time()
                self.latest_frame = image_msg_to_numpy(self.latest_frame)

            except:
                rospy.logerr(
                    "Current '{}' not ready yet, retrying for getting image".format(self.topic_name))
